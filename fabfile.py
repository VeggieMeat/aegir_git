...
def build_platform(site, profile, webserver, dbserver, giturl, gitbranch, build):
  print "===> Building the platform..."
  run("git archive --prefix=/var/aegir/platforms/%s --format=tar --remote=%s %s | tar -xvf -" % (build, giturl, gitbranch))
...
def install_site(site, profile, webserver, dbserver, giturl, gitbranch, build):
...
def migrate_site(site, profile, webserver, dbserver, giturl, gitbranch, build):
...
def save_alias(site, profile, webserver, dbserver, giturl, gitbranch, build):
...
def import_site(site, profile, webserver, dbserver, giturl, gitbranch, build):
